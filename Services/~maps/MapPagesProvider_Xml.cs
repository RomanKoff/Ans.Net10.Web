		...
		: IMapPagesProvider
	{
			...
			string virtualPath)
		{
			...
			if (!File.Exists(file1))
			{
				...
				Debug.WriteLine($"[Ans.Net10.Web] MapPagesProvider_Xml.GetMapPages(\"{node1}\") : MISSED");
				return null;
			}
			...
			try
			{
				var data1 = SuppXml.GetObjectFromXmlFile<MapPagesXmlRoot>(
					...
					"http://tempuri.org/Ans.Net10.Web.MapPages.xsd");
				var branch1 = _getBranch(data1.Items);
				map2 = new MapPages(branch1, virtualPath, node);
				Debug.WriteLine($"[Ans.Net10.Web] MapPagesProvider_Xml.GetMapPages(\"{node1}\") : LOADED");
			}
			catch (Exception)
			{
