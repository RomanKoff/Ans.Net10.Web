...
namespace Ans.Net10.Web.Services
{
		...
		: IMapNodesProvider
	{
			...
			string virtualPath)
		{
			...
			{
				_missed = true;
				Debug.WriteLine($"[Ans.Net10.Web] MapNodesProvider_Xml.GetMapNodes() : MISSED");
				return null;
			}
			try
			{
				var data1 = SuppXml.GetObjectFromXmlFile<MapNodesXmlRoot>(
					...
					"http://tempuri.org/Ans.Net10.Web.MapNodes.xsd");
				var branch1 = _getBranch(data1.Items);
				_map = new MapNodes(branch1, virtualPath);
				Debug.WriteLine($"[Ans.Net10.Web] MapNodesProvider_Xml.GetMapNodes() : LOADED");
			}
			catch (Exception)
			{
